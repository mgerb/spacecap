...
const VideoEditorSession = @import("./video_editor_session.zig").VideoEditorSession;
...
pub const VideoEditorStore = struct {
    ...
    pub const ExportTrimmedVideoPayload = struct {
    ...
    pub const Message = union(enum) {
        ...
        pub const effects = .{
            ...
            .export_trimmed_video = .{effect_export_trimmed_video},
        };
    ...
    pub const State = struct {
        ...
        /// VideoEditorSession is ref counted (with Arc) because it's accessed
        /// in effects, which may outlive the main store update thread cycle.
        ///
        /// WARNING: If a session is accessed outside of the state lock, then
        /// its ref count needs to be incremented with `clone`.
        sessions: std.array_hash_map.Auto(SessionId, Arc(VideoEditorSession)),
        active_session_id: ?SessionId = null,
        /// Incrementing unique ID whenever a new tab is selected. It feels a
        /// bit odd, but this is used to keep the UI in sync with the store.
        /// It's one of the downsides to immediate mode UIs. When this ID does
        /// not match with the local IMGUI ID, then it means that the active
        /// session tab needs to be selected.
        tab_selection_generation_id: u64 = 0,
        ...
        pub fn get_latest_display_frame(self: *@This()) !?VulkanImageRingBuffer.DisplayFrame {
            if (self.get_active_session()) |session| {
                return session.ring_buffer.get_latest_display_frame();
            }
            return null;
        }
        ...
        fn set_active_session(self: *@This(), session_id: ?SessionId) void {
            ...
            if (self.active_session_id != null) {
                self.tab_selection_generation_id += 1;
            }
            self.sync_session_activity();
        }

        fn close_session(self: *@This(), session_id: SessionId) void {
            const session = self.sessions.fetchOrderedRemove(session_id) orelse return;
            if (self.active_session_id == session_id) {
                self.active_session_id = null;
            }
            session.value.deinit();
            self.sync_session_activity();
        }
            ...
            .video_editor => |video_editor_msg| {
                switch (video_editor_msg) {
                    .open_session_success => |session| {
                        ...
                        if (state.video_editor.get_session_id_for_path(owned_session.as_ptr().file_path.bytes)) |session_id| {
                    ...
                    .export_trimmed_video_success, .export_trimmed_video_fail => |session_id| {
                        if (state.video_editor.sessions.get(session_id)) |session| {
                            session.as_ptr().is_exporting = false;
                        }
                    },
                }
            },
    ...
    pub fn effect_open_session(store: *Store, file_path: String) !void {
        ...
        const existing_session_id = blk: {
            ...
            break :blk state_locked.unwrap_ptr().video_editor.get_session_id_for_path(owned_file_path.bytes);
        };
        if (existing_session_id) |session_id| {
            log.debug("[effect_open_session] session already open: {s}", .{owned_file_path.bytes});
    ...
    fn effect_wake_worker(store: *Store, payload: anytype) void {
        var session = blk: {
            ...
            // Clone to increase the ref count while we have the state lock.
    ...
    fn effect_export_trimmed_video(store: *Store, payload: ExportTrimmedVideoPayload) !void {
        errdefer store.dispatch(.{
            .video_editor = .{ .export_trimmed_video_fail = payload.session_id },
        });
        ...
        var input_path: String = undefined;
        {
            const state_locked = store.state.lock();
            defer state_locked.unlock();
            const session = state_locked.unwrap_ptr().video_editor.sessions.get(payload.session_id) orelse {
        ...
        const output_directory = std.fs.path.dirname(input_path.bytes) orelse {
            log.err("[effect_export_trimmed_video] source file has no parent directory: {s}", .{input_path.bytes});
...
test "VideoEditorStore - init, scrub, and close all sessions" {
    ...
    const first_session_id = state.video_editor.sessions.keys()[0];
    const previous_selection_generation = state.video_editor.tab_selection_generation_id;
    store.dispatch(.{ .file_browser = .{
        .select_file = try .init(allocator, TestUtil.fixtures[0].file_path),
    } });
    store.run(.{ .once = true, .wait_for_effects = true });
    store.run(.{ .once = true, .wait_for_effects = true });
    try std.testing.expectEqual(first_session_id, state.video_editor.active_session_id.?);
    try std.testing.expectEqual(TestUtil.fixtures.len, state.video_editor.sessions.count());
    try std.testing.expect(state.video_editor.tab_selection_generation_id != previous_selection_generation);
