...
pub const VulkanVideoEncoder = struct {
    ...
    fn encode_video_frame(self: *Self) !EncodeResult {
        ...
        const gop_frame_count: u32 = @intCast(self.frame_count % GOP_LENGTH);
    ...
    /// Wait for the GPU fence, copy data off the GPU, and return the the raw encoded data.
    pub fn get_output_video_packet(self: *Self) ![]const u8 {
        defer {
