...
pub const c_def = struct {
    ...
    pub extern fn spa_pod_builder_pop(
        arg_builder: [*c]c.struct_spa_pod_builder,
        arg_frame: [*c]c.struct_spa_pod_frame,
    ...
    pub fn spa_pod_builder_add_object(
        ...
        var _f = std.mem.zeroes(c.spa_pod_frame);
        _ = c.spa_pod_builder_push_object(b, @ptrCast(&_f), type_, id);
        _ = @call(.auto, c.spa_pod_builder_add, .{b} ++ args ++ .{@as(i32, 0)});
